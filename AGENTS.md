...
- `dev2026/` contains the 2026 refactor API, store layer, time-cube tooling, and deployment runbooks. As of v0.5.0, VM24 production serves full-history points from the tiered time-cube and keeps the daily Zarr only as recent ingest/recovery staging.
...
- VM24 production data lives under `/home/odbadmin/Data/ghrsst/`. The authoritative serving index is the tiered time-cube: `mur_timecube_s8_t90_sh128.zarr` plus `mur_timecube_s8_t90_sh128.delta.zarr`. The daily store is recent ingest/recovery staging, not the full-history availability authority. As of v0.5.0 it contains 38 days; 1,267 historical day groups are in the rollback hold and have not been hard-deleted.
...
- Point availability is the chronological union of base+delta+daily, never `StoreAccess.bounds()` alone. Multi-day and single-day point queries normally route to the cube (`X-Store-Route: cube`). `mixed` is valid during ingest lag when a requested range includes cube history plus a daily-only newest day; it must preserve requested order and must not silently truncate.
...
- v0.4.1 production checkpoint: the first delta prune/swap retained 34 days (`2026-06-23..2026-07-26`) with measured API downtime of 2.791 seconds. See `dev2026/specs/p4s10_production_results.md`.
- v0.5.0 production checkpoint: the first daily-staging prune moved 1,267 historical groups to hold and retained 38 recent groups. A move into hold is a same-filesystem rename and frees **no disk space**; space is reclaimed only after explicit review and hard deletion after `hold_until`. The post-prune point-availability fix is commit `5099613`; see `dev2026/specs/p4s11_point_availability_regression.md`.
- The VM24 serving worktree contains deployment-specific files and does not have a Git HEAD that alone describes production. Compare module hashes/diffs against the intended release before applying a minimal patch. Do not checkout/reset the dirty runtime worktree. The v0.5.0 patch preserved VM24 Swagger-only changes in `api/app.py`.
- NGINX uses a shared API cache and caches 400/404 for one minute. After correcting an API error, an unchanged external URL can briefly return the old 400 while a cache-busted request already succeeds. Wait for the configured TTL and recheck; do not purge the shared cache indiscriminately.
