...
|---|---|---|---|---|---|---|
| 1 | 1 | 2.59 ms | **7.44 ms** | 1.00× | 38.7 ms | 7.1 ms |
| 4 | 4 | 2.50 ms | **13.08 ms** | 1.76× | 44.1 ms | 31.0 ms |
| 12 | 11 | 2.51 ms | **26.07 ms** | 3.50× | 61.4 ms | 77.4 ms |

*Generated by the harness and copied verbatim, and **enforced by
`TestResultsDocMatchesArtifact`** — run digest `d2a5a1cb789638ef`, matching [`p5s2_segmented_read.json`](../bench/results/p5s2_segmented_read.json). Hand-transcribing this table drifted on four review rounds; it is now emitted as `markdown_table`, and a tracked test recomputes the digest from the rows, requires the doc to cite it, requires this table verbatim, and checks every quoted figure — so hand-editing either file fails the suite instead of passing quietly.*

**Added cost per extra array call: median 0.6235 ms** (samples [0.621, 0.626] this run; ≈0.60–0.71 ms across repeated runs)
...
On this synthetic fixture a 366-day 3-variable read costs ~7.4 ms, so ~2 ms/call of Python+zarr
...
ms** for a 366-day range (v0.5.0). The same absolute overhead — 12 extra calls × ≈0.6235 ms ≈
**≈ 7.48 ms** at S = 90 — projects to roughly **+7.5–9.8 %** there. That is a projection, not a
...
**Snapshot-open cost** (7.1 → 31.0 → 77.4 ms for 1 → 4 → 12 segments) is roughly linear in
