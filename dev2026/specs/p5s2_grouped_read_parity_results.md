...
*Generated by the harness and copied verbatim — run digest `d2a5a1cb789638ef`, matching [`p5s2_segmented_read.json`](../bench/results/p5s2_segmented_read.json). Hand-transcribing this table drifted on four review rounds; it is now emitted as `markdown_table` so a doc quoting one run under another run's heading is detectable rather than plausible.*
