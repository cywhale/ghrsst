...
- Tests: [`../tests/test_phase2_p5s1.py`](../tests/test_phase2_p5s1.py) — **37/37 green**
- Full local suite: **315 tests OK** (17 skipped), up from 278; no regressions.
...
**PASS.** No such path was found. Every rejection keeps the previously-installed snapshot and
raises; a first build on an invalid manifest raises and serves nothing at all.
...
## 3. `SegmentedCubeStore` is already a `TieredCube` drop-in

Not required until S2, but proven now because it de-risks that step and because §6.1's whole
claim rests on it. `TieredCube(SegmentedCubeStore(...), TimeCubeStore(delta))` works unchanged:
...
`TieredCube` needed **no modification**: it consumes `_day_index` for membership and
`point_series` for reads, and the segmented store provides both.
