...
- [~] **P1-S5/S6 runbook authored** → [`specs/p1s5_s6_runbook.md`](specs/p1s5_s6_runbook.md)(VM24 deps/sizing/start、binding load 指令與 G* gate、NGINX cutover + `/points` location + cache landmine 修法 + `probe --strict` + purge/rollback)。**VM24 執行(binding gate + 部署)由 Codex/ops 進行,非 Claude**
